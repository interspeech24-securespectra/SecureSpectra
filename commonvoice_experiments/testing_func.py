...
def general_testing_func_EER(signModelDir = "signature_model.pt", verifyModelDir = "verification_model.pt", originalMelDataDir = "../models/original_CV_val/melspecs.npy", clonesMelDataDir= "/home/obiwan/repos/watermarking_module/cloned_audio/cloned_CV_val/melspecs.npy"):
    originalMelData = np.load(originalMelDataDir)
    originalMelLabels = np.zeros(originalMelData.shape[0])
    # code for the audio signing

    signedAudios = signData(signModelDir=signModelDir,melDataDir=originalMelDataDir, device=torch.device("cuda:0"))
...
general_testing_func_EER(signModelDir = "best_model_sign.pt", verifyModelDir = "best_model_verify.pt", originalMelDataDir = "./training_mel_commonvoice/original_CV_val/melspecs.npy", clonesMelDataDir= "./training_mel_commonvoice/cloned_CV_val/melspecs.npy")